...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.refreshSession = function (key) {
    ...
    return session.fetchToken(key)
      ...
      .then(function(results) {
        ...
        // Inject the list of userDBs
        if(typeof userDoc.personalDBs === 'object') {
          var userDBs = {};
          var publicURL;
          if(config.getItem('dbServer.publicURL')) {
            var dbObj = url.parse(config.getItem('dbServer.publicURL'));
            ...
            publicURL = config.getItem('dbServer.protocol') + newSession.key + ':' + newSession.salt + '@' +
              config.getItem('dbServer.host') + '/';
          }
          Object.keys(userDoc.personalDBs).forEach(function(finalDBName) {
            userDBs[userDoc.personalDBs[finalDBName].name] = publicURL + finalDBName;
          });
          newSession.userDBs = userDBs;
        }
  ...
  this.getUserDB = function(user_id, dbName, type) {
      var user;

      var p1 = new Promise((resolve, reject) => {
        userDB.get(user_id).then((userDoc) => {
          var user = userDoc;
          var db;
